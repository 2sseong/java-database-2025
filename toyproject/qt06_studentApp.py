# Oracle Student앱
# CRUD 데이터베이스 DML(SELECT, INSERT, DELETE)
...
class MainWindow(QMainWindow):
    ...
    def btnAddClick(self):
        ...
        if std_name == '' or std_regyear =='':
            ...
            if self.addData(values) == True: # 튜플을 파라미터로 전달
                QMessageBox.about(self, '저장성공','학생정보등록성공')
            else:
                QMessageBox.about(self, '저장실패','관리자에게 문의하세요')
            self.loadData() # 다시 테이블위젯데이터를 DB에서 조회
    ...
    def btnDelClick(self):
        ...
        if std_id == '':
            ...
            print('DB삭제진행')
            # Oracle은 파라미터 타입에 아주민감. 정확한 타입을 사용해야 함.
            values = (int(std_id), ) # 튜플로 전달, std_id 문자열에서 숫자로 변경
            if self.delData(values) == True:
                QMessageBox.about(self, '삭제성공','삭제했어요')
            else:
                QMessageBox.warning(self, '삭제실패','삭제 안돼요')
    ...
    def makeTable(self,lst_student):
        ...
        for i, (std_id, std_name, std_mobile, std_regyear) in enumerate(lst_student):
            self.tblStudent.setItem(i,0,QTableWidgetItem(str(std_id))) # oracle number타입은 뿌릴떄 str()로 형변환 필요
